(() => {
  ...
  function drawMaterial(w, h, t) {
    ...
    if (state.day === 1) {
      ...
      const drawCoil = (limit, completed, front) => { for (let step = 0; step < Math.ceil(steps * limit); step += 1) { const p0 = step / steps; const p1 = Math.min(limit, (step + 1) / steps); const mid = (thetaAt(p0) + thetaAt(p1)) / 2; const isFront = Math.sin(mid) > 0; if (isFront !== front) continue; ctx.setLineDash(front ? [] : [3, 4]); ctx.lineWidth = front ? (completed ? 4.2 : 2.9) : (completed ? 2.15 : 1.65); ctx.strokeStyle = front ? `rgba(245,226,158,${completed ? .86 : .58 + pulse * .16})` : `rgba(188,156,91,${completed ? .62 : .48})`; ctx.beginPath(); ctx.moveTo(xAt(p0), yAt(p0)); ctx.lineTo(xAt(p1), yAt(p1)); ctx.stroke(); } };
