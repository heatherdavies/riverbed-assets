...
The display additionally applies a **vertical presentation overscan**: it samples only the settled central 85% of the same live height texture. The upper and lower 7.5% of the simulation are kept beyond the visible view, so the current enters the screen already formed and leaves below the frame. This crop applies only to height and normal lookups. The riverbed retains original screen coordinates and is never translated or scrolled.

A final **gentle downstream styling cue** adds a low-amplitude teal light only to the already-live, downward-moving crest faces. It is based on the crest mask, surface slope, and existing height texture; it does not change simulation physics, texture coordinates, or riverbed sampling. Lateral meander and trailing ripple layers are deliberately negligible, because competing bright crest directions make the water’s overall flow ambiguous. The crests are not an animated background layer or a scrolling texture. The display intentionally excludes animated background images, contour diagnostics, independently scrolling caustics, broad cellular patterns, and moving topographic-line treatments.
...
| `scripts/patch-gentle-downstream-styling.js` | Adds a subtle downstream-facing crest-light cue in the final display shader only; it does not change simulation or riverbed sampling. |
